...
contract UmaCtfAdapter is Auth, ReentrancyGuard {
    ...
    constructor(address _ctf, address _finder) {
        ...
        optimisticOracle = OptimisticOracleInterface(
            FinderInterface(_finder).getImplementationAddress(UmaConstants.OptimisticOracle)
        );
    ...
    /// If reward > 0 or proposal bond, caller must have allowance and rewardToken to pay for the price request and bond
    /// Prepares the condition using the Adapter as the oracle and a fixed outcomeSlotCount
    ...
    function initializeQuestion(
        ...
        // Initialize the question on the adapter
        _initializeQuestion(questionID, ancillaryData, requestTimestamp, rewardToken, reward, proposalBond);
    ...
    function getExpectedResolutionData(QuestionData storage questionData) internal view returns (int256) {
        return
            getOptimisticOracle()
                .getRequest(address(this), UmaConstants.YesOrNoIdentifier, questionData.requestTimestamp, questionData.ancillaryData)
                .resolvedPrice;
    }
    ...
    function supportedToken(address token) internal view returns (bool) {
        return collateralWhitelist.isOnWhitelist(token);
    }
}
